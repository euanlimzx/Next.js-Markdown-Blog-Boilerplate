...
- Stage: the scheduler splits the logical plans into stages at exchange boundaries.
- Exchange boundaries: They occur in a particular type of plan that involves some exchange of data. In our codebase, the Join operator requires an exchange boundary because it needs to collect the output of all involved worker tasksbefore performing a join
- Task: A stage is split further into tasks to exploit data parallelism in our engine. Tasks are what is executed in workers. In our engine, for parquet files with multiple row groups, we emit one task per row group to enable intra-file parallelism

When the scheduler receives a plan, it runs a **distributed scheduler** that breaks the plan down either into a singular stage (if there's no need for an exchange boundary), or multiple stages (if there's an exchange boundary required).

- In the event of a single stage plan, we have a **task manager** that splits this stage into multiple row-group sub tasks, before calling the worker \_\_\_\_

- In the event of a multi-stage plan,
  1. we have a **task manager** that splits the stages BEFORE the exchange boundary into multiple row-group sub tasks.

  2. We have a **stage coordinator** that submits all tasks in parallel, and we perform a distributed shuffle (more on that later!).

  3. We have a barrier that waits for all the workers the complete their task before

-

##### The worker management service

The worker management service keeps track of all available workers we could possibly assign tasks to. It exposes a service that let's workers register themselves. Workers periodically call this service as a form of "heartbeat" that lets the scheduler know the worker is still alive and available to receive tasks.
...
- **Metrics** answer the question of "How is our system behaving?". We track two different types of metrics:
...
| --------------- | -------------------------------------------- | --------------------------------------------- |
| **Trace** | The full journey of one request, end-to-end | A single flight itinerary |
| **Span** | One unit of work within that trace | One leg of the flight |
| **TraceID** | 128-bit ID shared by every span in one trace | Booking reference number |
| **SpanID** | 64-bit ID unique to one span | Leg number within that booking |
| **Parent span** | The span that created this span | The outbound flight that connects to this leg |
| **Root span** | The first span in a trace (no parent) | The first leg |

**In our engine:** The root span is created in the scheduler when a query first arrives. Every span after that (stages, worker tasks) is a child of it.

```
[Trace: query_id = abc-123]
│
├── [Span: do_get]                          ← Scheduler receives query
│   │   duration: 250ms
│   │   query_id: abc-123
│   │
│   ├── [Span: execute_stages]              ← StageCoordinator runs stages
│   │   │   stage_count: 3
│   │   │
│   │   ├── [Span: execute_shuffle_map]     ← Worker A does shuffle
│   │   │       worker_id: worker-1
│   │   │       rows: 4000
│   │   │
│   │   └── [Span: execute_shuffle_map]     ← Worker B does shuffle
│   │           worker_id: worker-2
│   │           rows: 3800
│   │
│   └── [Span: execute_task (join)]         ← Join worker assembles result
│           worker_id: worker-1
│           rows_out: 7800
```
...
The tracing subscriber is a \_\_\_\_ that allows you to subscribe to spans from the `tracing` crate. This operates at an engine level, allowing you to decide what to do with the spans collected throughout the engine

**Layer 3: OpenTelemetry (OTEL)**

Open telementry is a vendor-neutral standard for observability. It defines a data model for traces, metrics and logs, as well as a wire protocol for exporting that data (OTLP). This gives you the ability to instrument once, but swap higher level layers easily.
...
For Jaeger to show one unified trace, the Worker must know that its traces are children of the Scheduler's tracers so that they can create a child span, even though everything runs in a different process. We solve this by injecting context into gRPC metadata headers, bringing us to the next layer.
...
Jaeger receives both spans, sees the TraceID matches, and draws them as parent-child in one unified flame graph.
